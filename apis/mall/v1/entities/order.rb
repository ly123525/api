      ...
      class Order < Grape::Entity
        ...
        expose :inviting_friends_info do |m, o|
          if m.fight_group.present? && m.fight_group.waiting?
            ...
            {
              url: "http://39.107.86.17:8080/#/mall/products?uuid=#{m.order_items.first.product.uuid}&fight_group_uuid=#{m.try(:fight_group).try(:uuid)}",
              image: image,
              title: "来拼",
              summary: "来拼"
            }
          ...
          if o[:fight_group]
            {
              ...
              url: "http://39.107.86.17:8080/#/mall/products?uuid=#{m.order_items.first.product.uuid}&fight_group_uuid=#{o[:fight_group].uuid}",
              description: '快来拼单吧'
            }
          else
            {
