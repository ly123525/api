    ...
    module Mall
      ...
      class Order < Grape::Entity
        ...
        expose :to_refund_scheme do |m, o|
           "lvsent://gogo.cn/web?url=" + Base64.urlsafe_encode64("http://39.107.86.17:8080/#/after_sale?order_uuid=#{m.uuid}") unless (m.created? || m.closed? || m.refunded? || m.fight_group.try(:waiting?))
        end  
      ...
      class Orders < Grape::Entity
        ...
        expose :buy_again_scheme do |m, o|
          "lvsent://gogo.cn/mall/products?style_uuid=#{m.try(:order_items).try(:first).try(:style).try(:uuid)}" if m.received? or m.evaluated? or m.closed? or m.paid?
        end
